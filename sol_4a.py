...
class Network(layers.BaseNetwork):
    ...
    def __init__(self, data_layer, hidden_units, hidden_layers):
        ...
        self.MY_MODULE_LIST.append(layers.Bias(self.MY_MODULE_LIST[-1]))
        self.MY_MODULE_LIST.append(layers.Sigmoid(self.MY_MODULE_LIST[-1]))
...
class Trainer:
    ...
    def setup(self, training_data, parameters=None):
        ...
        self.loss_layer = layers.CrossEntropy(
            self.network.get_output_layer(), y)
...
def main(test=False):
    ...
    if not test:
        ...
        dict_train_test = data_generators.data_4a()
        ...
        num_correct, num_total = public_tests.test_network_arch(
            "sol_4a", test_data, test_answers)

        count += num_correct

        print("Architecture", num_correct, "/", num_total)

        num_correct, num_total = public_tests.test_final_acc(
            "sol_4a", test_data, test_answers)
        ...
        print("Total Correct", count, "Total Tests", "3\n")

        # if plot == 1:
        #     test_data = {'trainer': trainer}
        #     experiments.plot_tvp("sol_4a", test_data,
