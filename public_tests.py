...
def test_mnist_acc(module_name, test_data, test_answers):
  ...
  trainer.network.load_state_dict(
      np.load("mnist_weight.npz", allow_pickle=True)["weight"])
...
def test_gradients(module_name, test_data, test_answers):
  """
  This code inspired by Tom G's `check_gradient` function in `utility.ipynb`
  """
  ...
  for iter in range(10):
    ...
    delta_f_approx = np.array([(g * hh).sum()
                               for g, hh in zip(grad_w, h)]).sum()
    delta_f = delta_f.sum()
...
if __name__ == "__main__":
  ...
  solution_directory = '/Users/harduin/Desktop/OFFLINE CLASSES STUFF/Semester 2/828L/Problem Sets/Problem Set 1/Testing Stuff/The Good Stuff'
