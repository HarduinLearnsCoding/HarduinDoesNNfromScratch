...
def data_1b():
    ...
    x1_train = x1_all[::2]
    x2_train = x1_all[::2]
    x_train = np.array(np.meshgrid(x1_train, x2_train)
                       ).transpose().reshape(-1, 2)
    A = np.array([[2, -3]])
    b = np.array([5])
    y_train = np.einsum("ij,...j->...i", A, x_train) + \
        np.random.randn(900, 1) * 0.1
    # gen test data
    x1_test = x1_all[1::2]
    x2_test = x2_all[1::2]
    x_test = np.array(np.meshgrid(x1_test, x2_test)).transpose().reshape(-1, 2)
    y_test = np.einsum("ij,...j->...i", A, x_test) + \
        np.random.randn(900, 1) * 0.1
    return {
        "train": (x_train, y_train),
        "test": (x_test, y_test)
...
def data_2b():
    def compute_y(grid):
        y = 1.7 * grid[:, 0:1]**2 + 0.6 * grid[:, 1:2] + 2 + np.random.randn(900,
                                                                             1) * 0.1
        return y
    # generating data
...
if __name__ == "__main__":
    # generate_default_data("sol_1a")
    dataset = data_4b()
    train = dataset["train"]
    test = dataset["test"]
    (xtest, ytest) = test
    print("Test N-D Shape", ytest.shape)
