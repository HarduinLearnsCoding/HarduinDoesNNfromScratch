...
def main(test=False):
    ...
    if not test:
        ...
        dict_train_test = data_generators.data_3a()
        ...
        # if plot == 1:
        #     test_data = {'trainer': trainer}
        #     experiments.plot_tvp("sol_3a", test_data,
        #                          trainer.get_num_iters_on_public_test())

        # For prob mnist: ex'numpy.ndarray' object has no attribute 'out_dims'ample data loading code:
        # mnist = np.load("mnist.pkl", allow_pickle=True)
        # x_train = mnist["training_images"]s
